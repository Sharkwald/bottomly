...
public class GiphyEventHandlerTests
{
    private readonly GiphyEventHandler _handler;
    private readonly Mock<ISlackMessageBroker> _mockBroker = new();
    private readonly Mock<GiphyCommand> _mockCommand;

    public GiphyEventHandlerTests()
    {
        var options = TestHelpers.CreateOptions();
        var mockFactory = new Mock<IHttpClientFactory>();
        _mockCommand = new Mock<GiphyCommand>(mockFactory.Object, options);
        _handler = new GiphyEventHandler(_mockCommand.Object, _mockBroker.Object, options,
            NullLogger<GiphyEventHandler>.Instance);
    }
