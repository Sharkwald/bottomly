...
public class GetLeaderBoardEventHandlerTests
{
    private readonly GetLeaderBoardEventHandler _handler;
    private readonly Mock<ISlackMessageBroker> _mockBroker = new();
    private readonly Mock<IKarmaRepository> _mockKarmaRepo = new();

    public GetLeaderBoardEventHandlerTests()
    {
        var options = TestHelpers.CreateOptions();
        var command = new GetLeaderBoardCommand(_mockKarmaRepo.Object);
        _handler = new GetLeaderBoardEventHandler(command, _mockBroker.Object, options,
            NullLogger<GetLeaderBoardEventHandler>.Instance);
    }
