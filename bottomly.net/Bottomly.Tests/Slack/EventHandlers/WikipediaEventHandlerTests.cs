...
public class WikipediaEventHandlerTests
{
    private readonly WikipediaEventHandler _handler;
    private readonly Mock<ISlackMessageBroker> _mockBroker = new();
    private readonly Mock<WikipediaSearchCommand> _mockCommand;

    public WikipediaEventHandlerTests()
    {
        var options = TestHelpers.CreateOptions();
        var mockFactory = new Mock<IHttpClientFactory>();
        _mockCommand = new Mock<WikipediaSearchCommand>(mockFactory.Object);
        _handler = new WikipediaEventHandler(_mockCommand.Object, _mockBroker.Object, options,
            NullLogger<WikipediaEventHandler>.Instance);
    }
