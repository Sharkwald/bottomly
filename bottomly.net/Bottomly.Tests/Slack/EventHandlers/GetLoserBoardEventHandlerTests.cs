...
public class GetLoserBoardEventHandlerTests
{
    private readonly GetLoserBoardEventHandler _handler;
    private readonly Mock<ISlackMessageBroker> _mockBroker = new();
    private readonly Mock<IKarmaRepository> _mockKarmaRepo = new();

    public GetLoserBoardEventHandlerTests()
    {
        var options = TestHelpers.CreateOptions();
        var command = new GetLoserBoardCommand(_mockKarmaRepo.Object);
        _handler = new GetLoserBoardEventHandler(command, _mockBroker.Object, options,
            NullLogger<GetLoserBoardEventHandler>.Instance);
    }
