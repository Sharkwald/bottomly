...
public class GoogleEventHandlerTests
{
    private readonly GoogleEventHandler _handler;
    private readonly Mock<ISlackMessageBroker> _mockBroker = new();
    private readonly Mock<GoogleSearchCommand> _mockCommand;

    public GoogleEventHandlerTests()
    {
        var options = TestHelpers.CreateOptions();
        _mockCommand = new Mock<GoogleSearchCommand>(options);
        _handler = new GoogleEventHandler(_mockCommand.Object, _mockBroker.Object, options,
            NullLogger<GoogleEventHandler>.Instance);
    }
