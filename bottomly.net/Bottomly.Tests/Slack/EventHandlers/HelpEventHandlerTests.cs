...
public class HelpEventHandlerTests
{
    private readonly Mock<ISlackMessageBroker> _mockBroker = new();

    private HelpEventHandler CreateHandler(IEnumerable<IEventHandler>? handlers = null)
    {
        var options = TestHelpers.CreateOptions();
        return new HelpEventHandler(
            handlers ?? Enumerable.Empty<IEventHandler>(),
            _mockBroker.Object,
            options,
            NullLogger<HelpEventHandler>.Instance);
    }
    ...
    {
        var mockH1 = new Mock<IEventHandler>();
        mockH1.Setup(h => h.BuildHelpMessage()).Returns("Handler1 Help");
        var mockH2 = new Mock<IEventHandler>();
